...
function Deck(opt) {
  ...
  // 设置输入语言
  this.inputLang = _.get(opt, 'inputLang');
  if (_.isUndefined(this.inputLang)) {
    throw '"inputLang" undefined';
  }
  ...
  if (opt.hasOwnProperty('file')) {
    ...
    this.list  = this.getList(fs.readFileSync(this.file).toString());
  }
  else {
    // @todo check
    this.title = _.get(opt, 'title', moment().format('YYYYMMDD_HHmmss'));

    this.list = this.getList(opt.list);
  }
...
Deck.prototype.dlPdf = function() {
  ...
  var imgFilesPromises = _.mapValues(self.list, function(v, k) {
    return DB.findImgByTitleAndLang(k, self.inputLang);
  });

  return new Promise(function(resolve, reject) {
    return BPromise.props(imgFilesPromises)
    .then(function(imgs) {
      ...
      _.forIn(imgs, function(imgPath, title) {

        if (_.isNull(imgPath)) return;
